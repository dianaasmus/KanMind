...
async function init() {
  ...
  renderMemberList();
  renderTitle();
  if (getParamFromUrl("task_id"))
    openTaskDetailDialog(getParamFromUrl("task_id"));
}
...
function renderDetailTask() {
  document.getElementById("task_detail_dialog_select").value =
    currentTask.status;
  document.getElementById("detail_task_title").innerHTML = currentTask.title;
  document.getElementById("detail_task_description").innerHTML =
    currentTask.description;
  document.getElementById("detail_task_assignee").innerHTML =
    getDetailTaskPersonTemplate(currentTask.assignee);
  document.getElementById("detail_task_reviewer").innerHTML =
    getDetailTaskPersonTemplate(currentTask.reviewer);
...
async function postComment(element) {
  ...
  if (newComment.content.length > 0) {
    let response = await postData(
      TASKS_URL + currentTask.id + "/comments/",
      newComment
    );
    if (!response.ok) {
...
function renderDetailTaskComments() {
  ...
  document.getElementById(
    "comment_sender_profile"
  ).innerHTML = `<div class="profile_circle color_${userInitials[0]}">${userInitials}</div>`;
...
function deleteComment(id) {
  deleteData(TASKS_URL + currentTask.id + "/comments/" + id + "/").then(
    async (response) => {
      if (!response.ok) {
        let errorArr = extractErrorMessages(response.data);
        showToastMessage(true, errorArr);
...
function renderTaskCreateMemberList() {
  document.getElementById("create_edit_task_assignee").innerHTML =
    getTaskCreateMemberListEntrieTemplate("assignee", currentBoard);
  document.getElementById("create_edit_task_reviewer").innerHTML =
    getTaskCreateMemberListEntrieTemplate("reviewer", currentBoard);
...
function getValidatedTask() {
  ...
  if (
    validateCreateEditTaskTitle(titleRef) &&
    validateCreateEditTaskDueDate(dateRef)
  ) {
    let updatedTask = {
      board: currentBoard.id,
      title: titleRef.value,
      description: document.getElementById("create_edit_task_description")
        .value,
...
function setSelectAddEditTaskStatusDropdown() {
  document.getElementById("create_edit_task_dialog_select").value =
    currentTask.status;
}
...
function resetAllMoveOpen() {
  document
    ...
    .forEach((btn) => btn.setAttribute("move-open", "false"));
}
...
function fillCreateEditTaskTitleInputDesc() {
  document.getElementById("create_edit_task_title_input").value =
    currentTask.title;
  document.getElementById("create_edit_task_date_input").value =
    currentTask.due_date;
  document.getElementById("create_edit_task_description").value =
    currentTask.description;
}
...
async function openEditBoardDialog() {
  document
    .getElementById("edit_board_dialog")
    .setAttribute("current_dialog", "true");
